...
namespace {/**
 * Nette Framework (version 2.0-dev b8dac97 released on 2011-01-04)
 *
...
applyLimit(&$sql,$limit,$offset);}}namespace Nette\Forms{use
Nette;interface
...
new
NotImplementedException('Offset is not implemented.');}}}class
PdoMySqlDriver
extends
Nette\Object
implements
...
applyLimit(&$sql,$limit,$offset){if($limit<0&&$offset<1)return;$sql.=' LIMIT '.($limit<0?'18446744073709551615':(int)$limit).($offset>0?' OFFSET '.(int)$offset:'');}}class
PdoOciDriver
extends
Nette\Object
implements
...
applyLimit(&$sql,$limit,$offset){if($offset>0){$sql='SELECT * FROM (SELECT t.*, ROWNUM AS "__rnum" FROM ('.$sql.') t '.($limit>=0?'WHERE ROWNUM <= '.((int)$offset+(int)$limit):'').') WHERE "__rnum" > '.(int)$offset;}elseif($limit>=0){$sql='SELECT * FROM ('.$sql.') WHERE ROWNUM <= '.(int)$limit;}}}class
PdoOdbcDriver
extends
Nette\Object
implements
...
new
InvalidArgumentException('Offset is not implemented in driver odbc.');}}class
PdoPgSqlDriver
extends
Nette\Object
implements
...
new
NotImplementedException;}function
applyLimit(&$sql,$limit,$offset){if($limit>=0)$sql.=' LIMIT '.(int)$limit;if($offset>0)$sql.=' OFFSET '.(int)$offset;}}class
PdoSqliteDriver
extends
Nette\Object
implements
...
applyLimit(&$sql,$limit,$offset){if($limit<0&&$offset<1)return;$sql.=' LIMIT '.$limit.($offset>0?' OFFSET '.(int)$offset:'');}}class
PdoSqlite2Driver
extends
PdoSqliteDriver{function
formatLike($value,$pos){throw
new
NotSupportedException;}}}namespace Nette\Database\Reflection{use
Nette;class
DatabaseReflection
extends
...
__construct($statement){}function
count(){return
...
as$key=>$row){$this->rows[isset($row[$this->primary])?$row[$this->primary]:$key]=new
...
REVISION='b8dac97 released on 2011-01-04';public
