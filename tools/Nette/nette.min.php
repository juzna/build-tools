...
class
Framework{const
NAME='Nette Framework',VERSION='2.0-dev',REVISION='3961bef released on 2011-04-15';public
...
Nette\FatalErrorException($error['message'],0,$error['type'],$error['file'],$error['line'],NULL));return;}if(self::$showBar&&!self::$productionMode&&!self::$ajaxDetected&&!self::$consoleMode&&!preg_match('#^Content-Type: (?!text/html)#im',implode("\n",headers_list()))){Helpers::renderDebugBar(self::$panels);}}static
function
...
<?php }}else{if(self::$consoleMode){echo"$exception\n";}elseif($htmlMode){Helpers::renderBlueScreen($exception);}elseif(!self::fireLog($exception,self::ERROR)){self::log($exception);}}foreach(self::$onFatalError
